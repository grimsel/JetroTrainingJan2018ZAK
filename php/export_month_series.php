...
function create_and_export_pdf_series()
{
	...
	if($isSessionActive)
	{
		...
		$pdfAuthor = "Badging_Team";
						...
						// mktime Monate + 1 = richtiger Monat
						$export_info = '<hr><h1></h1><h1></h1><h2>Monatsbericht Badging vom ' . $de_monate[$y] . ' ('. date("Y") .') von '. $name .'</h2>';
						...
						while($outputData = $badgeQueryMonth->fetch(PDO::FETCH_ASSOC)) 
						{
							...
							// PDF Titelinfo
							$export_art = "Monatsbericht_von_";
							$pdfName = date("Y").'_' . $de_monate[$y] . "_Badging_". $export_art . $name . ".pdf";
							...
							$html[$countHTML] .= '<tr>
										...
										if($vorherigerName !== htmlspecialchars($outputData['Vorname']) . " " . htmlspecialchars($outputData['Nachname']) && $vorherigerName != "")
										{
											...
											$pdf[$countPDF]->Output('/home/jetro_admin/public_html/pdf_export_month/' . $pdfName, 'F');
											ob_end_clean();
											$countPDF++;
											$countHTML++;
										}
						$vorherigerName = htmlspecialchars($outputData['Vorname']) . " " . htmlspecialchars($outputData['Nachname']);
						$name = htmlspecialchars($outputData['Vorname']) . "_" . htmlspecialchars($outputData['Nachname']);
						...
						$zipName = "../" . date("Y") . "_Monatsberichte_vom_" . $de_monate[$y] . ".zip"; //  Beim Pfad wird über den Ordner "php" gegangen wegen Berechtigungen
						$dir = '/home/jetro_admin/public_html/pdf_export_month/';
						...
						{
								$status = $zip->open($zipName, ZipArchive::OVERWRITE); // Zip Archiv überschreiben
